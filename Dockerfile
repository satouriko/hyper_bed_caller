FROM rust:1.39
RUN apt-get update && apt-get install -y build-essential libcurl3 libssl-dev zlib1g-dev gperf cmake git
RUN git clone -b 'v1.5.0' --single-branch --depth 1 https://github.com/tdlib/td.git /td
RUN cd /td && mkdir build && cd build \
    && cmake -DCMAKE_INSTALL_PREFIX="/usr" -DCMAKE_BUILD_TYPE=Release .. && cmake --build . \
